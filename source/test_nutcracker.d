...
unittest { write("tests/nutcracker.d "); }
...
unittest {
	write("#4 ");
	struct TestStructV2 { double[] val; int answer; bool flag; }
	...
	struct TestStructV1 { double[] val; int answer; }
	static foreach (T; EnumMembers!MsgBufferType) {{
		auto msg = toMsgBuffer!(T)(t1);
...
unittest {
	...
	struct TestStructEnriched { TestStruct val; ulong timestamp; }

	// the final, i.e. receive end struct simply merges both together
	struct TestStructFinal { double[] val; ulong timestamp; }
...
unittest {
	...
	// this is the "original" message
	struct TestStruct { double[] val; ulong timestamp; }
	...
	struct TestStructFinal { double[] val; }
...
unittest {
	...
	struct TestStructOld { ulong[] val; ulong timestamp; }
	...
	static foreach (T; EnumMembers!MsgBufferType) {{
		...
		struct TestStructNew { ulong[] val; ulong timestamp; string greeting; double temp; }
